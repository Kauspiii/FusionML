...
on Llama-3-8B and GPT-2 XL decoder blocks (inference only).

Separates the FusionML speedup into two orthogonal contributions:
  - FP16 precision alone        (half the memory bandwidth, GPU compute)
  - mx.compile graph fusion     (JIT kernel fusion, reduced dispatch overhead)
  - Combined (FusionML default)
...
MODES = [
    ("FP32-NoCompile", dict(fp16=False, compiled=False)),
    ("FP16-NoCompile", dict(fp16=True,  compiled=False)),
    ("FP32-Compiled",  dict(fp16=False, compiled=True)),
...
def run_worker(model, fp16, compiled):
    """Run one (model, fp16, compiled) config; return stats dict or None."""
    ...
    if model == "llama":
        ...
        def _llama_fwd(x, w_q, w_k, w_v, w_o, w_gate, w_up, w_down, ln1_g, ln1_b, ln2_g, ln2_b):
            Dh = x.shape[-1]
        ...
        _fn = mx.compile(_llama_fwd) if compiled else _llama_fwd

        def step(x_in):
            xd = x_in.astype(dtype)
            res = _fn(xd, w_q, w_k, w_v, w_o, w_gate, w_up, w_down, ln1_g, ln1_b, ln2_g, ln2_b)
            mx.eval(res)

    else:  # gpt2
        w_fc1 = _arr((D, F)); w_fc2 = _arr((F, D))

        def _gpt2_fwd(x, w_q, w_k, w_v, w_o, w_fc1, w_fc2, ln1_g, ln1_b, ln2_g, ln2_b):
            Dh = x.shape[-1]
            h  = mxf.layer_norm(x, ln1_g, ln1_b, 1e-5)
            q  = h @ w_q; k = h @ w_k; v = h @ w_v
            scale = 1.0 / (Dh ** 0.5)
        ...
        _fn = mx.compile(_gpt2_fwd) if compiled else _gpt2_fwd

        def step(x_in):
            xd = x_in.astype(dtype)
            res = _fn(xd, w_q, w_k, w_v, w_o, w_fc1, w_fc2, ln1_g, ln1_b, ln2_g, ln2_b)
            mx.eval(res)
...
def print_ablation_table(model_name, results):
    ...
    for label, _ in MODES:
        ...
        print(f"  {label:<20} | {s['mean']:>9.2f} ± {s['ci95']:>6.2f}   | {s['median']:>12.2f} | {ratio:>12.2f}× | {s['peak_mem_mb']:>9.0f}{marker}")
...
def main():
    ...
    print("=" * 70)
    print("  FusionML FP16 × Compile Ablation Study")
    ...
    for model_key, model_name in [("llama", "Llama-3-8B"), ("gpt2", "GPT-2 XL")]:
        ...
        for label, flags in MODES:
            ...
            if s:
                ...
                print(f"{s['median']:.2f} ms  (×{(1.0 if label=='FP32-NoCompile' else all_results[model_key].get('FP32-NoCompile',{}).get('median',s['median'])/s['median']):.2f})")
            else:
    ...
    out_path = os.path.join(out_dir, "fp16_ablation.json")
